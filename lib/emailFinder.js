...
async function findEmail({ firstName, lastName, domainOrCompany }) {
    ...
    const mxHosts = dnsInfo.mxRecords?.map(r => r.exchange).filter(Boolean) || (dnsInfo.preferredMx ? [dnsInfo.preferredMx] : []);

    const [scraperResult, isCatchAll] = await Promise.all([
        scrapeWebsite(domain),
        checkCatchAll(domain, mxHosts),
    ]);
    ...
    const smtpResults = await verifyEmails(patterns.map(p => p.email), mxHosts);

    const smtpWorked = smtpResults.some(r => r.status === 'valid' || r.status === 'invalid');
    if (!smtpWorked) {
        meta.warnings.push('SMTP verification inconclusive across MX hosts. Using alternative signals.');
    }
    ...
    return {
        ...
        meta: {
            ...
            smtpUnknown: smtpResults.filter(r => r.status === 'unknown').length,
            smtpErrors: smtpResults.filter(r => r.status === 'error').length,
